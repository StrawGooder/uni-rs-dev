							...
							<view class="next-tree-item-block" v-if="item.show">
								<view class="next-tree-item" :style="[{
								 ...
								 :class="{
									...
									@tap.stop="onTreeItemClicked(item, index)"
									...
									<template>
										<!-- zs adding -->
										<slot name="item-end" 
										v-bind:data = "_makeItemEndSlotParam(item,index)">
...
<script>
	import { createActionTrigger } from '../../../../utils/actions';
	...
	export default {
		...
		created(){
			// this.rfCurClickedItemId = 0
			this._cur_click_item_id = 0
			this._temp_clicked_count = 0;
			this._dbclick_action_trig = createActionTrigger(
											"TimesTrigger", 
											this.onTreeItemDbclicked, 
											256,
											{times:2}
											)
			// this._dbclick_action_trig.addListener("rejected", this._treeItemTap)
			// when rejected event emited, fallback to response  that item was clicked once
			this._dbclick_action_trig.addListener("lessrejected", this._treeItemTap)
		},
		...
		methods: {
			...
			async onTreeItemClicked(item, index){
				
				this._dbclick_action_trig.increase()
				this._dbclick_action_trig.setArgs(item)
				// this._treeItemTap(item, _index)
			},
			// 点击
			...
			_initTree(){
				...
				// console.log("debug-ntree", this.treeData.slice(0,2))
			...
			// zs-adding
			_makeItemEndSlotParam(item, index){
